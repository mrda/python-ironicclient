...
class ClientTest(utils.BaseTestCase):
    ...
    @mock.patch.object(filecache, 'retrieve_data')
    @mock.patch.object(ironicclient.client, '_get_ksclient')
    def _get_client_with_api_version(self, version, mock_gk, mock_rd):
        mock_gk.return_value = fake_get_ksclient()
        kwargs = {
            'os_tenant_name': 'TENANT_NAME',
            'os_username': 'USERNAME',
            'os_password': 'PASSWORD',
        ...
        client = get_client('1', **kwargs)
        self.assertEqual(0, mock_rd.call_count)
    ...
    @mock.patch.object(filecache, 'retrieve_data')
    @mock.patch.object(ironicclient.client, '_get_ksclient')
    def test_get_client_default_version_set_no_cache(self, mock_gk, mock_rd):
        mock_gk.return_value = fake_get_ksclient()
        ...
        client = get_client('1', **kwargs)
        self.assertEqual(1, mock_rd.call_count)
    ...
    @mock.patch.object(filecache, 'retrieve_data')
    @mock.patch.object(ironicclient.client, '_get_ksclient')
    def test_get_client_default_version_set_cached(self, mock_gk, mock_rd):
        version = '1.3'
        # Make sure we don't coincidentally succeed
        self.assertNotEqual(v1.DEFAULT_VER, version)
        mock_gk.return_value = fake_get_ksclient()
        mock_rd.return_value = version
        kwargs = {
            'os_tenant_name': 'TENANT_NAME',
            'os_username': 'USERNAME',
            'os_password': 'PASSWORD',
